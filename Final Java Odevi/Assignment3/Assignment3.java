...
"The path to new adventures is now open!");
        System.out.println(" ");
        System.out.println("Two skilled adventurers, Swiftclaw and Ironhide, are impressed\n" +
"by Pyre’s strength and decide to join the team!");
        System.out.println(" ");
    ...
    public static void triggerEvent(int regionIndex, int scenarioIndex, int simulatedChoice, String[] teamMemberNames, int[] teamMemberCurrentHP, int[] teamMemberMaxHP, int[] teamMemberAP, int[] teamMemberDP, boolean[] teamMemberAlive){
        ...
        if(regionIndex == 0 && scenarioIndex == 0){
            if(simulatedChoice == 1){
                ...
                System.out.println("Outcome: The team fights back bravely! They take " + damage + " damage in the\n" +
"skirmish. " + teamMemberNames[targetIMAX] + "’s resolve is hardened, increasing its AP by 5.");
                int [] totalTeamStats = calculateTotalTeamStats(teamMemberCurrentHP , teamMemberAP , teamMemberDP , teamMemberAlive);
                displayTeamStats(teamMemberNames, teamMemberCurrentHP, teamMemberMaxHP, teamMemberAP, teamMemberDP, teamMemberAlive, totalTeamStats);
            }
        ...
        if(regionIndex == 1 && scenarioIndex == 1){
            ...
            if(simulatedChoice == 2){
                ...
                System.out.println("Outcome: The team harvests the crystals, feeling a surge of vitality.\n" +
"Their maximum and current HP increase!");
                int [] totalTeamStats = calculateTotalTeamStats(teamMemberCurrentHP , teamMemberAP , teamMemberDP , teamMemberAlive);
                displayTeamStats(teamMemberNames, teamMemberCurrentHP, teamMemberMaxHP, teamMemberAP, teamMemberDP, teamMemberAlive, totalTeamStats);
            }
        }
        if(regionIndex== 2 && scenarioIndex == 0){
            if(simulatedChoice == 1){
                ...
                System.out.println("It’s a trap! The mechanism releases a burst of negative\n" +
"energy, damaging the team.");
                int [] totalTeamStats = calculateTotalTeamStats(teamMemberCurrentHP , teamMemberAP , teamMemberDP , teamMemberAlive);
                displayTeamStats(teamMemberNames, teamMemberCurrentHP, teamMemberMaxHP, teamMemberAP, teamMemberDP, teamMemberAlive, totalTeamStats);
            }
            else{
