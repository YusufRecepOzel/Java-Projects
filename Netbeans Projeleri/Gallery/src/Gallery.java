...
public class Gallery {
    ...
    String[] buyCars = new String[cars.length + 1];
    double[] buyPrices = new double[cars.length + 1];
    for(int i = 0 ; i < cars.length ; i++){        
        buyCars[i] = cars[i];
        buyPrices[i] = prices[i];
    }
    
    buyCars[cars.length] = carName;
    buyPrices[cars.length] = price;
    ...
    public static void sellCar(String carName){
        ...
        String [] sellCars = new String[cars.length - 1];
        double [] sellPrices = new double[cars.length - 1];
        int k = 0;
